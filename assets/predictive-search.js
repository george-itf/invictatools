...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const searchWrapper = document.querySelector(".trade-header__search");
  const results = document.querySelector(".ps-results");

  if (!psRoot || !searchWrapper || !results) return;

  psRoot.appendChild(results);
  ...
  document.addEventListener("click", (e) => {
    if (!results.contains(e.target) && !searchWrapper.contains(e.target)) {
      results.classList.remove("ps-results--visible");
    }
  });

  const input = document.querySelector(".ps-input");
  if (input) {
    input.addEventListener("input", () => {
      results.classList.add("ps-results--visible");
      positionDropdown();
    });
  }
